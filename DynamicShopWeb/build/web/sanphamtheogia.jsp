                        ...
                        <jsp:include page="slider.jsp"></jsp:include>	
                            ...
                            <div id="content">
                                ...
                                <%
                                    ...
                                    int count = qlcsdl.dsProduct().size();
                                    int pages = p.findPages(count, limit);
                                    String sql = "";
                                    String queryString = "";
                                    if (Integer.parseInt(request.getParameter("type")) == 1) {
                                        sql = "select * from product order by price desc limit " + start + "," + limit + "";
                                        queryString = "&type=1";
                                    }
                                    if (Integer.parseInt(request.getParameter("type")) == 0) {
                                        sql = "select * from product order by price asc limit " + start + "," + limit + "";
                                        queryString = "&type=0";
                                    }
...
%>
                                <div class="cl"></div>
                                <div class="paging">
                                    <h3><%=p.nextPrevQueryString(selectPage, pages, request.getRequestURI(), queryString)%></h3>
                                    <h3><%=p.pageListQueryString(selectPage, pages, request.getRequestURI(), queryString)%></h3>
                                </div> 
                                <%
                                    }// end if
%>
